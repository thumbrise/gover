...
import (
	...
	discovery2 "github.com/thumbrise/multimod/internal/model/states/discovery"
	"github.com/thumbrise/multimod/internal/testutil"
)

func newDiscovery() *discovery2.Discovery {
	return discovery2.NewDefaultDiscovery()
}
...
func TestDiscover_CrossDeps(t *testing.T) {
	...
	if !slices.Contains(betaReplaces, "example.com/root") {
		t.Errorf("beta.Replaces missing root: %v", betaReplaces)
	}

	if !slices.Contains(betaReplaces, "example.com/root/alpha") {
		t.Errorf("beta.Replaces missing alpha: %v", betaReplaces)
	}
}
...
func TestDiscover_CyclicDeps_Error(t *testing.T) {
	root := testutil.Scaffold(t, "cyclic_deps")
	d := newDiscovery()
...
func TestDiscover_CorruptedGoMod_Error(t *testing.T) {
	root := testutil.Scaffold(t, "corrupted_gomod")
	d := newDiscovery()
...
func TestDiscover_ExcludedDirs(t *testing.T) {
	...
	// _tools/ is now visible (workspace-only convention), vendor/ and testdata/ remain excluded.
	if len(state.Subs) != 1 {
		t.Errorf("len(Subs) = %d, want 1 (_tools/ visible, vendor+testdata excluded)", len(state.Subs))
	}
}

func TestDiscover_NestedSub(t *testing.T) {
	root := testutil.Scaffold(t, "nested_sub")
	d := newDiscovery()
...
func TestDiscover_EmptyGoMod_Error(t *testing.T) {
	root := testutil.Scaffold(t, "empty_gomod")
	d := newDiscovery()
...
func TestDiscover_RootRequiresSub_Error(t *testing.T) {
	root := testutil.Scaffold(t, "root_requires_sub")
	d := newDiscovery()
