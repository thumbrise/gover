...
import (
	...
	"github.com/thumbrise/multimod/internal/applier"
	"github.com/thumbrise/multimod/internal/model"
	"github.com/thumbrise/multimod/internal/model/states/discovery"
	publishsteps "github.com/thumbrise/multimod/internal/model/states/publish"
	"github.com/thumbrise/multimod/internal/testutil"
)
...
func TestApply_PublishState_StripsReplacesAndPinsVersions(t *testing.T) {
	root := testutil.Scaffold(t, "valid_project")

	// First apply dev-state (normal discovery).
	devState := discover(t, root)

	a := applier.NewApplier()
	if err := a.Apply(devState); err != nil {
		t.Fatalf("Apply dev-state failed: %v", err)
	}

	// Build publish-state: strip replaces, pin requires.
	publishPipeline := model.NewPipeline(publishsteps.NewDefaultPipeline("v1.2.3"))

	publishState, err := publishPipeline.Run(devState)
	if err != nil {
		t.Fatalf("Publish pipeline failed: %v", err)
	}

	// Apply publish-state.
	if err := a.Apply(publishState); err != nil {
		t.Fatalf("Apply publish-state failed: %v", err)
	}
	...
	// Replaces must be stripped.
	if strings.Contains(content, "replace") {
		t.Errorf("publish-state go.mod still has replace directive:\n%s", content)
	}

	// Internal requires must be pinned to v1.2.3.
	if !strings.Contains(content, "v1.2.3") {
		t.Errorf("publish-state go.mod missing pinned version v1.2.3:\n%s", content)
	}
}

// discover runs the full discovery pipeline and fails on error.
func discover(t *testing.T, root model.AbsDir) model.State {
	t.Helper()
