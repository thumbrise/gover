...
- **Full ecosystem name** — a type inside a package must carry the full name of its role, not a shortened form that relies on the package name for context. `applier.GoWorkApplier`, not `applier.GoWorkSync`. `goinvoker.GoInvoker`, not `goinvoker.Invoker`. The type name must be self-describing when read in isolation — in error messages, logs, stack traces, and cross-package references. If a type is an extension of the package's core concept, the core concept must be in the type name.
- **Facade + internal** — when a package is an ecosystem (multiple types collaborating behind one public API), the public type is the facade and implementation details live in `internal/` subpackages. The facade delegates, the internals do the work. Outsiders see only the facade. Example: `applier.Applier` is the facade; `applier/internal/goworkapplier.GoWorkApplier` and `applier/internal/gomodapplier.GoModApplier` are invisible implementation details.
- **Flat package vs ecosystem package** — if a directory contains a simple set of types that share one concern and no type needs its own internal details, it's a flat package (one package, multiple files). But if it contains multiple independent ecosystems — each with its own facade, internal operations, helpers — it must be a set of packages. Each ecosystem declares its own semantic boundary. Signs you need separate packages: types don't share state, autocomplete leaks unrelated symbols, or one type needs `internal/` but another doesn't.
